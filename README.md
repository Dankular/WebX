...
┌──────────────────────────── Browser tab ─────────────────────────────────┐
│                                                                            │
│  ┌──────────────── Canary  (x86-64 Linux VM in WASM) ───────────────────┐ │
│  │                                                                        │ │
│  │   Steam Game  (.exe)                                                   │ │
│  │     │  Win32 / DirectX 9–12 API calls                                 │ │
│  │     ▼                                                                  │ │
...
│  │     │  outb/inl on x86 I/O port 0x7860 (emulated by Canary)           │ │
│  └─────┼──────────────────────────────────────────────────────────────────┘ │
│        │  Canary registerPortListener  MessagePort channel                   │
│        ▼                                                                     │
...
|-----------|--------------|---------|------|
| **Canary** | `D:\Dev Proj\Canary` (this workspace) | Browser (WASM) | x86-64 Linux VM; boots real SteamOS ext2 image; emulates IN/OUT port instructions |
...
| **VKD3D-Proton** | [HansKristian-Work/vkd3d-proton](https://github.com/HansKristian-Work/vkd3d-proton) | Canary VM | Translates D3D12 → Vulkan; ships inside Proton |
| **libvkwebx.so** | `guest-icd/` (this repo) | Canary VM (x86-64) | Thin Vulkan ICD; serializes every Vulkan call to binary wire packets |
...
harness/
  ...
  canary-host.mjs       Canary boot, SteamOS image mount, IPC bridge setup
...
over a Canary x86 I/O port channel (port 0x7860).
...
### 1. Browser — Canary (production target)

The primary target. Boots the SteamOS ext2 image inside a browser tab via Canary's
x86-64 WASM emulator. All GPU commands cross the I/O port bridge to the browser-side
VkWebGPU-ICD plugin.

```sh
npm run build:canary    # build Canary WASM (requires Rust + wasm-pack)
...
# 4. Inside the VM: build + install VkWebGPU-ICD natively
...
sudo cp vkwebgpu_icd.json /etc/vulkan/icd.d/

# 5. Launch a game via Proton
...
```sh
npm run extract:image    # decompress .bz2, extract rootfs partition (Windows, Python)
...
```sh
npm run build:canary     # build Canary WASM first
npm run dev              # start browser dev server at http://localhost:3000
```

Open `http://localhost:3000` and click **Launch**.
...
| Browser harness (`harness/`) | ✅ Canary boot + IPC bridge |
| VkWebGPU-ICD plugin stub | ✅ Full WebGPU translation layer |
| SteamOS image preparation | ✅ Docker + WSL scripts |
| Canary x86 IN/OUT port emulation | 🔄 In progress (Canary dev) |
...
| [Canary](../../Canary) | x86-64 Linux VM in WASM; runs the SteamOS image in the browser |
...
| **Rust + wasm-pack** | Not needed | Required (Canary WASM build) |
